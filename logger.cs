...
namespace _100HperWeekTimer
{
    ...
    public static class DailyLogger
    {
        ...
        private static bool isEqualDay = false;
        ...
        public static void dispose()
        {
            using (FileStream fileStream = File.Open(directory, FileMode.Create))
            {
                ...
                {
                    fileStream.Write(Encoding.UTF8.GetBytes(lines[i] + "\n"));
                }
                if (!isEqualDay)
                {
                    fileStream.Write(Encoding.UTF8.GetBytes(lines[lines.Length - 1]));
                }
